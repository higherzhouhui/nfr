...
import {getMyANftList} from '@/services/user';
...
const User: NextPage = () => {
  ...
  useEffect(() => {
    if (status.list && status.list.length) {
      const arr: any = [];
      status.list.forEach((item: any) => {
        arr.push({
          ...item,
          checked: false,
        });
      });
      options[2].children = [...arr];
      setOptions([...options]);
    }
  }, [status]);
  useEffect(() => {
  ...
  const getList = async (pageNo: number = 1) => {
    ...
    options[2].children.forEach((child: any) => {
      if (child.checked) {
        status.push(child.value);
      }
    });
    const res: any = await getMyANftList({
      pageNo,
      pageSize,
